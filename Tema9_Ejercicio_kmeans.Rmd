...
Otra manera
(Encontrada aquí:  https://warin.ca/posts/rcourse-clustering-with-r/)
...
```{r, fig.height=5}
fviz_nbclust(datos_scaled, kmeans, method = "gap_stat")
fviz_nbclust(datos_scaled, kmeans, method = "wss")
```
Estas 2 gráficas no me parece que ayuden demasiado a decidir el número de clusters.
...
Vamos a examinar también el resultado con 3 clústeres:
...
fviz_cluster(clusters3, data = datos_scaled, show.clust.cent = TRUE, ellipse.type = "convex", palette = palette, ggtheme = theme_minimal(), main = "Resultados Acidosis")
```
...
Si elegimos 5 clústeres, ese paciente es el primero que efectivamente es separado de los demás:
...
fviz_cluster(clusters5, data = datos_scaled, show.clust.cent = TRUE, ellipse.type = "convex", palette = palette, ggtheme = theme_minimal(), main = "Resultados Acidosis")
```
