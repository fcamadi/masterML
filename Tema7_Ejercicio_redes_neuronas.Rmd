...
if (library == "keras") {
  X_train_bank <- bank_processed_train %>% 
    select(-y) %>% 
    keras3::as_tensor(select(-y), dtype = "float32")  #c("y","y.yes","y.no") ?
...
if (library=="neuralnet") {
  ...
  #model <- keras_model_sequential(name = "keras_40_10", input_shape = 40, dtype="float32") 
  model <- keras_model_sequential(name = "keras_1_2")
  model %>%
    layer_dense(name = "layer_1", units = 1, activation = 'relu', input_shape = ncol(X_train_bank)) %>%
...
En el caso de utilizar la librería Keras&TensorFlow, podemos obtener un Json con información del modelo:
...
Pues el 100 % al primer intento:

keras_model_sequential(name = "keras_40_10_2")
   user  system elapsed 
224.615  15.739 270.078 
...
Vamos a intentar simplificar el modelo, y reducir el valor de epoch
(Han sido menos de 4 minutos con este modelo).

Con solo 2 capas, tarda solo 24 s, para el mismo resultado, con 40 y 20 neuronas obtiene el 100%.

keras_model_sequential(name = "keras_40_2")
   user  system elapsed 
 23.974   2.023  28.094
...
Con 10+2 neuronas, mismo resultado en la mitad de tiempo (epoch=50)
   user  system elapsed 
 12.703   0.865  14.253 
 ...
 (El tiempo no baja de 10-12 segundos)
 
Para empeorar el resultado, hay que disminuir el valor de epoch.
Con 1+2 neuronas, y epoch=25
