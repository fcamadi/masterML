...
svm_liblinear_28k_best <- readRDS("svm_liblinear_all_datasets_28k_freq20_cost01_bias10_weights_1_2.rds")
...
```{r}
train_vocab_28k_best <- colnames(svm_liblinear_28k_best$W)

subset1_sparse_matrix_28k_best <- process_unlabelled_posts(subset1, train_vocab_28k_best)
```
...
predictions_28k_best <- predict(svm_liblinear_28k_best, subset1_sparse_matrix_28k_best)
...
subset1$label_28k_best <- predictions_28k_best$predictions
```
...
```{r}
table(subset1$label_28k_best)
prop.table(table(subset1$label_28k_best))
```
...
```{r}
result1_1000$label_28k_best <- subset1$label_28k_best

write.table(result1_1000, "subset1_classification_SVM_100k_600k_and_28kbest.csv", sep = "|")
```
...
#Confusion matrix
confusionMatrix(reference = as.factor(result1_1000$label), 
                data = as.factor(result1_1000$label_28k_best), 
                positive="1", 
                mode = "everything")
```
