...
101 x 101 sparse Matrix of class "ngCMatrix"
  [[ suppressing 101 column names ‘{dual incomes=yes,householder status=own,income3levels=50k+}’, ‘{marital status=married,occupation=professional/managerial,language in home=english,income3levels=50k+}’, ‘{occupation=professional/managerial,householder status=own,language in home=english,income3levels=50k+}’ ... ]]
  [[ suppressing 101 column names ‘{dual incomes=yes,householder status=own,income3levels=50k+}’, ‘{marital status=married,occupation=professional/managerial,language in home=english,income3levels=50k+}’, ‘{occupation=professional/managerial,householder status=own,language in home=english,income3levels=50k+}’ ... ]]
...
101 x 101 sparse Matrix of class "ngCMatrix"
  [[ suppressing 94 column names ‘{dual incomes=yes,householder status=own,income3levels=50k+}’, ‘{marital status=married,occupation=professional/managerial,language in home=english,income3levels=50k+}’, ‘{occupation=professional/managerial,householder status=own,language in home=english,income3levels=50k+}’ ... ]]
  [[ suppressing 94 column names ‘{dual incomes=yes,householder status=own,income3levels=50k+}’, ‘{marital status=married,occupation=professional/managerial,language in home=english,income3levels=50k+}’, ‘{occupation=professional/managerial,householder status=own,language in home=english,income3levels=50k+}’ ... ]]
                                               ...
                                               {occupation=student,dual incomes=not married,language in home=english,income3levels=0-20k$} 
                                                  ...
                                                  {marital status=single,occupation=student,dual incomes=not married,income3levels=0-20k$} 
                                                                        ...
                                                                        {occupation=student,dual incomes=not married,income3levels=0-20k$} 
                                                                                                                                     FALSE 
{marital status=married,householder status=own,type of home=house,ethnic classification=white,language in home=english,income3levels=50k+} 
                                                                                                                                     FALSE 
...

]

Las reglas marcadas como TRUE son consideradas redundantes por lo que podemos eliminarlas de los resultados:
...
La 35 por ejemplo indica que una edad entre 25 y 34, si vive de alquiler, es porque probablemente tendrá un sueldo que le permite vivir de manera independiente (ni en el domicilio de los padres .. ni adquirir una propiedad).
...
Si la ocupación es estudiante, entonces la regla de asociación con un sueldo bajo tiene una elevación una confianza todavía mayores (regla 14).
(Y mayores todavía si la regla incluye en la parte izquierda que se está solter y se es estudiante, regla 7).

En cuanto a los sueldos más altos:
Si hay dos sueldos en la unidad familiar, y se tiene la propiedad de la vivienda (regla 1), eso indica que es bastante probable que sea porque los sueldos son altos.
...
interestMeasure(rules_pruned, c("support",  "confidence", "lift", "chiSquare",  "coverage"), income3L_complete)
```

Los valores de "chi squared" para las 26 primeras reglas son muy altos (también son altos para las últimas 10). 
Esto indica una relación muy fuerte entre la parte izquierda y derecha de las reglas.
Entiendo que las reglas de asociación obtenidas tienen validez (no son triviales).


### Visualización de las reglas

Con la ayuda de la librería arulesViz, vamos a visualizar las reglas obtenidas, para ver si podemos obtener algo más de información:
...
Tuve que disminuirlo hasta poder encontrar reglas de asociación relacionadas con los niveles de ingreso.


Gráfico de grupos entre reglas:
