...
```{r}
# import the CSV file
odio_kaggle_test_raw <- read.csv(file.path("dataset_04_kaggle.csv"), sep=";")  # -> 1243 obs. (only spanish) of 3 variables
...
```{r}
system.time({
  liblinear_svm_model_c <- LiblineaR(data = posts_freq_train_mat, target = odio_kaggle$label, type = 3,  cost = c) 
...
En cambio, eligiendo un coste por encima de 1 (1.5, 2, y 3), se mejora el resultado. Elegimos el mejor coste = 3.
...
system.time({
  ...
  liblinear_svm_model_weights <- LiblineaR(data = posts_freq_train_mat, target = odio_kaggle$label, type = 3,
                                   wi = class_weights)
})
```
...
Probando diferentes combinaciones del parámetro coste y de los pesos, se obtiene siempre una exactitud que ronda el 76-77% y un kappa del 0.47%.
