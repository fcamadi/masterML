---
title: "hate_speech_02"
author: "Fran Camacho"
date: "2025-06-04"
output: word_document
---


# TFM - Procesar datasets


```{r}
source('hate_speech_common.R')
```


Carga de paquetes que son necesarios para diversas funciones.

```{r}
load_libraries()    
```



# Paso 1: obtención del dataset y procesado preliminar


```{r}
# import the CSV file
odio_kaggle_raw <- read.csv(file.path("dataset_03_kaggle.csv"), sep=";")  # -> 1243 obs. (only spanish) of 3 variables
odio_kaggle_raw <- odio_kaggle_raw[-1]
```



Estructura del dataset:

```{r}
str(odio_kaggle_raw)
```


La columna "label" es de tipo int. Ya que se trata en realidad de una variable categórica 0/1, es conveniente transformarla en un factor:

```{r}
#Convert class into a factor
odio_kaggle_raw$label <- factor(odio_kaggle_raw$label)
```


Examinamos el resultado

```{r}
table(odio_kaggle_raw$label)
```

Este dataset está desbalanceado, pero no de una manera tan exagerada como los otros.


```{r}
head(odio_kaggle_raw)
```

```{r}
tail(odio_kaggle_raw)
```


# Paso 2: procesado del dataset


Como primer paso, se van a eliminar que referencien a otros usuarios en los comentarios. 
Estos son referenciados mediante la "@".

También, con ayuda de la librería cleantext examinamos los comentarios y haremos un primer procesado:

```{r}
#check_text(odio_kaggle_raw$post)

odio_kaggle <- preprocess_posts(odio_kaggle, odio_kaggle_raw)
```


Comprobamos el resultado:

```{r}
odio_kaggle[1:5,1]

#check_text(odio_kaggle$post)
```


### Corpus


Creación del objeto corpus con todas los mensajes:

```{r}
#create corpus
posts_corpus <- VCorpus(VectorSource(odio_kaggle$post))
print(posts_corpus)
```


Limpieza de los textos del corpus:

```{r}
system.time({

  posts_corpus_clean <- clean_corpus(posts_corpus)

})
```

(<6 s)


Finalmente se procede a la **"tokenización"** de los comentarios. Se obtiene una DTM:

```{r}
system.time({
  posts_dtm <- DocumentTermMatrix(posts_corpus_clean)
})

posts_dtm
```


Ahora hay que crear los conjuntos de entrenamiento y de test.

Dato que este dataset ya viene separado en dos conjuntos así, lo que hacemos es cargar el dataset de test:


```{r}
# import the CSV file
odio_kaggle_test <- read.csv(file.path("dataset_04_kaggle.csv"), sep=";")  # -> 1243 obs. (only spanish) of 3 variables
odio_kaggle_test <- odio_kaggle_test[-1]
```

[

```{r}
result <- train_test_split(odio_kaggle, posts_dtm, .75)
```


```{r}
posts_dtm_train <- result$dtm_train
posts_dtm_test <- result$dtm_test
posts_train_labels <- result$train_labels
posts_test_labels <-  result$test_labels
```



```{r}
#Set seed to make the process reproducible
set.seed(123)

#partitioning data frame into training (75%) and testing (25%) sets
train_indices <- createDataPartition(odio_kaggle$label, times=1, p=.75, list=FALSE)

#create training set
posts_dtm_train <- posts_dtm[train_indices, ]

#create testing set
posts_dtm_test  <- posts_dtm[-train_indices, ]

#create labels sets
posts_train_labels <- odio_kaggle[train_indices, ]$label
#posts_train_labels <- as.numeric(as.factor(posts_train_labels))

posts_test_labels <- odio_kaggle[-train_indices, ]$label
#posts_test_labels <- as.numeric(as.factor(posts_test_labels))

#view number of rows in each set
nrow(posts_dtm_train)  # 
nrow(posts_dtm_test)   # 
length(posts_train_labels)  # 
length(posts_test_labels)   # 
```

]


Se necesita ahora obtener un listado con las palabras más utilizadas:


```{r}
# Data preparation – creating indicator features for frequent words
# the function findFreqTerms() in the tm package takes a DTM and returns a character vector containing words that appear at least a  minimum number of times
posts_freq_words <- findFreqTerms(posts_dtm_train, 10)

#posts_freq_words
print("tonto" %in% posts_freq_words)    
print("imbecil" %in% posts_freq_words)  # <- FALSE with freq 1000
print("cabron" %in% posts_freq_words)   # <- FALSE with freq  150
```

   [1] "¿cómo"           "¿por"            "¿que"            "¿qué"            "¿te"             "abajo"           "abierta"         "abr"            
   [9] "absoluta"        "abuela"          "abuelo"          "abuso"           "acá"             "acaba"           "acabar"          "acabo"          
  [17] "acaso"           "ácido"           "acoso"           "acto"            "acuerdo"         "acusacion"       "acusado"         "ademá"          
  [25] "adversario"      "aficion"         "afición"         "africano"        "agent"           "agresión"        "agua"            "aguantar"       
  [33] "ahi"             "ahí"             "ahora"           "ahr"             "alegro"          "aleman"          "alemania"        "algo"      
  ...
  985] "propio"          "provocar"        "prueba"          "psoe"            "pública"         "público"         "pue"             "pueblo"         
 [993] "pued"            "pueda"           "pueden"          "puedo"           "puerta"          "puesto"          "puigdemont"      "punto"          
 [ reached 'max' / getOption("max.print") -- omitted 344 entries ]


Y ahora utilizamos ese listado para limitar el número de columnas/features de los conjuntos de entrenamiento y de test:

```{r}
ncol(posts_dtm_train)
posts_dtm_freq_train <- posts_dtm_train[ , posts_freq_words]
posts_dtm_freq_test <- posts_dtm_test[ , posts_freq_words]     
ncol(posts_dtm_freq_train)

```



## Paso 3: Entrenamiento del modelo


Tanto caret como LIBSVM (e1071) han probado ser muy poco eficientes a la hora de tratar dataset de estos tamaños.

Utilizamos por tanto LiblineaR también con este dataset.


```{r}
# dtm -> matrix -> df

posts_freq_train_mat <- as.matrix(posts_dtm_freq_train)
#posts_freq_train_df <- as.data.frame(posts_freq_train_mat)

posts_freq_test_mat <- as.matrix(posts_dtm_freq_test)
#posts_freq_test_df <- as.data.frame(posts_freq_test_mat)
```



```{r}
system.time({

  liblinear_svm_model <- LiblineaR(data = posts_freq_train_mat, target = posts_train_labels, type = 3)  # C = 1, not the found with heuristicC

})
  
```


```{r}
# prediction
system.time({
  
  prediction_liblinear <- predict(liblinear_svm_model, posts_freq_test_mat)

})
```


```{r}
#Confusion matrix
confusionMatrix(reference = as.factor(posts_test_labels), data = as.factor(prediction_liblinear$predictions), positive="1", mode = "everything")
```

- Intentamos mejorar el modelo usando el coste calculado por la función heurística:

Intentamos obtener un parámetro de coste con la función heuristicC, como aconsejan los creadores de esta librería.

```{r}
c <- heuristicC(posts_freq_train_mat)
c
```

```{r}
system.time({
  liblinear_svm_model_c <- LiblineaR(data = posts_freq_train_mat, target = posts_train_labels, type = 3,  cost = c) 
    
  prediction_liblinear_c <- predict(liblinear_svm_model_c, posts_freq_test_mat)
})
```
```{r}
confusionMatrix(reference = as.factor(posts_test_labels), data = as.factor(prediction_liblinear_c$predictions), positive="1", mode = "everything") 
```


El resultado es algo mejor, tanto en exactitud como en el valor de kappa. Pero se han detectado menos mensajes de odio.


- Intentamos mejorar el modelo usando pesos, para favorecer la detección de la clase minoritaria:


```{r}
# Define class weights
class_weights <- c("0" = 1, "1" = 3)  # Assign higher weight to the minority class
```


```{r}
system.time({

  liblinear_svm_model <- LiblineaR(data = posts_freq_train_mat, target = posts_train_labels, type = 3,
                                   wi = class_weights)
})
```


```{r}
# prediction
system.time({
  prediction_liblinear <- predict(liblinear_svm_model, posts_freq_test_mat)
})
```

```{r}
#Confusion matrix
confusionMatrix(reference = as.factor(posts_test_labels), data = as.factor(prediction_liblinear$predictions), positive="1", mode = "everything")
```





-------------------------------------------------------------------------------------------------------------



[
  Este dataset obtenido en kaggle tiene un dataset de test de acompañante. 
  Utilizaremos el dataset anterior para entrenamiento y validación, y este de test para test ?

  Lo importamos:

```{r}
# import the CSV file
odio_kaggle_test <- read.csv(file.path("dataset_04_kaggle.csv"), sep=";")  # -> 1243 obs. (only spanish) of 3 variables
odio_kaggle_test <- odio_kaggle_test[-1]
```


  Hay que procesarlo de igual manera que el dataset de entrenamiento
  para poder crear los conjuntos de entrenamiento y test.

]
