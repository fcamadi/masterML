...
if (!complete_dataset) {
  ...
  n  <- nrow(df_no_hate)
  k  <- 288936      # number of random rows 88936
  ...
  odio_hatemedia_300k <- rbind(df_no_hate_sample, df_hate)

  #so we don't have to change variable names
  odio_hatemedia_raw <- odio_hatemedia_300k
} else {
...
posts_freq_words_train <- findFreqTerms(posts_dtm_train, 500)  #   100 -> ~17000 terms
                                                          #   500 ->  ~6000
                                                          #  1000 ->  ~3600 
...
NO Probamos primero con LIBSVM (e1071)
...
Se intenta obtener el mejor coste con la función heuristicC:
...
Con el sub dataset de 300k, se obtiene una exactitud muy alta.
Kappa es por el contrario bastante mediocre.
...
class_weights <- c("0" = 1, "1" = 5)  # Assign higher weight to the minority class
```
